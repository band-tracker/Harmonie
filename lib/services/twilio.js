  ...
  User  
    ...
    .then(user => {
      ...
      return client.messages.create({
        to: newNum,
        from: process.env.MY_TWILIO_NUMBER,
        body: `${user.username}, you're now a member of ${band.name}, practice makes permanent...`
      });
    });
};
  ...
  User  
    ...
    .then(user => {
      ...
      return client.messages.create({
        to: newNum,
        from: process.env.MY_TWILIO_NUMBER,
        body: `${user.username}, you didn't make the cut for ${band.name}, not everyone is perfect...`
      });
    });
};
